...
2. 여러 PDA가 기본 URL에 접속하면 대기 작업을 서로 겹치지 않게 한 건씩 자동 선점
3. 작업자는 안내된 FROM → 품번 → TO 순서로 QR을 스캔하고, 완료 후 다음 작업을 자동 배정받음
4. 품번을 먼저 스캔하는 예외 흐름은 `?mode=item-first` URL에서만 사용
5. 불일치 또는 중복 적치 시: FAIL(빨강+경고음+장진동) → 정위치 스캔 전까지 차단
