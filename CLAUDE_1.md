...
**작업자는 품번 QR만 먼저 스캔하며, FROM 로케이션 QR은 스캔하지 않음**

```
[STEP 1] 품번 QR 스캔

  → item_code = 스캔값으로 active 이동지시 조회
  → 매핑 확인 후 placement_logs에서 이미 완료된 로케이션 복원 (앱 재시작 대응)
  → 같은 품번의 여러 FROM·TO를 한 화면에 합쳐 표시
  → 매핑이 없으면 "매핑 없음" 안내 후 품번 재스캔
...
[STEP 2 · 이동지침]
  품번과 연결된 FROM·TO·수량 표시
  FROM QR은 스캔하지 않음
  TO 로케이션 QR을 하나씩 스캔

[PASS]
  전체 적치 완료
  [다음 작업]을 누르면 품번 스캔으로 복귀
```
